...
public class Calc {
    ...
    public Complex[][] subMatrix(int noTouchI, int noTouchJ, Complex[][] c1) {
    ...
    public Complex matrixNorm(Complex[][] c1){
        Complex temp = new Complex(0,0);
        for (int i = 0; i < c1.length; i++) {
            for (int j = 0; j < c1[0].length; j++) {
                Complex pow = mult(c1[i][j], c1[i][j]);
                 temp = sum(temp,pow);
            }
        }
         Complex ans = 
    ...
    public Complex[][] accion(Complex[][] c1, Complex[][] v1) {
        ...
        for (int i = 0; i < v1.length; i++) {
            result = new Complex();
            for (int j = 0; j < c1[0].length; j++) {
                result = sum(result, mult(c1[i][j], v1[0][j]));
            }
            action[i][0] = result;
        }
        return action;
    ...
    public boolean isHermitia(Complex[][] c1) {
    ...
    public boolean unitary(Complex[][] c1) {
        boolean isUnitary = true;
        Complex[][] transpose = this.transpose(c1);
        Complex[][] ans = this.mult(c1, transpose);
        Complex one = new Complex(1, 0);
        for (int i = 0; i < ans.length && isUnitary; i++) {
            if (!ans[i][i].equals(one))
                isUnitary = false;
